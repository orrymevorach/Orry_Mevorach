...
export default function GetFormElement({
  ...
  switch (type) {
    case 'row':
      return (
        <div className={clsx(styles.formRow, inputContainerClassNames)}>
          {items?.map((elementConfig, index) => (
            <GetFormElement
              key={`${index}-submission-form`}
              {...elementConfig}
              inputClassNames={elementConfig.inputClassNames}
              labelClassNames={elementConfig.labelClassNames}
              inputContainerClassNames={elementConfig.inputContainerClassNames}
            />
          ))}
        </div>
      );
